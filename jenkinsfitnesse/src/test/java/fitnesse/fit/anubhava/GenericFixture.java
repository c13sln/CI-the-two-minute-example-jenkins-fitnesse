...
/**
 * Modified or written by Anubhava Srivastava for use with FitNesse. Copyright (c) 2008 Anubhava Srivastava Released under the terms of the GNU General Public
 * License version 2 This Generic class can be used to expose all the methods of a class using reflection APIs. Only condition is that all of the parameters to
 * these methods must be convertible from String using TypeAdapter.parse(String) API OR else a class must provide public static Object parse(String) method to
 * initialize itself. An example of such a class is com.thoughtworks.selenium.Selenium Method names and parameter values are supplied at run time from FitNesse
...
public class GenericFixture extends Fixture {
   ...
   protected class RowEntry {
      ...
      public void addText( String text ) {
         if( this.startReturnVals == 0 ){
            /*
             * //add a new cell Parse cell = new Parse("td", text, null, null); this.cells.get(this.cells.size()-1).more = cell; this.cells.add(cell);
             */
   ...
   protected RowEntry readRow( Parse row ) {
      ...
      if( re.ignored == false ){
         DSLAdapter.CommandRow cr = DSLAdapter.findMatch( re.methodName, methodParameters.toArray( new String[0] ) );
         if( cr != null ){
            re.methodName = cr.getMethodName();
            re.changeSUT();
            methodParameters = cr.getParameters();
            debug( "Found matching command: " + re.methodName + " " + methodParameters );
         }else
   ...
   public static Object fetchVariable( String var ) {
      ...
      if( text != null && text.length() > 1 && text.charAt( text.length() - 1 ) == EQ && text.indexOf( '.' ) == -1 )
   ...
   public static boolean hasEqualsMethod( Class<?> type ) {
      ...
      try{
         if( type != null && !type.equals( Object.class ) ){
            Method m = type.getDeclaredMethod( "equals", new Class[] { Object.class } );
   ...
   public static boolean callEqualsMethod( Class<?> type, Object o1, Object o2 ) throws Exception {
      boolean result = false;

      TypeAdapter ta = TypeAdapter.adapterFor( type );
      if( ta != null && !ta.getClass().equals( TypeAdapter.class ) ){
         result = ta.equals( o1, ta.parse( o2.toString() ) );
      }else{
         Method m = type.getMethod( "equals", new Class[] { Object.class } );
   ...
   private Object parseParameter( String parameter, Class<?> paramType ) throws Exception {
   ...
   /*
    * Here we try to lookup a class from a given name and create an instance from given parameters. Child class can use this method to set an object on which
    * reflection will invoke all methods
    */
      ...
      if( targetClass == null && args != null && args.length > 0 ){
         ...
         for( Constructor<?> constructor : constructors ){
            ...
            for( int p = 0; p < numParams; p++ ){
               try{
                  methodParameters[p] = parseParameter( args[p + 1], parameterTypes[p] );
               }catch( NullPointerException npe ){
                  nextConst = true;
                  break;
               }
      ...
      try{
         ...
         while( rows != null  ){
            ...
            if( re.ignored ){
               ...
               continue;
            }else if( re.varName != null && re.changedSUT != null && re.methodName.equals( THIS ) ){
            ...
            for( Method method : methods ){
               if( method.getName().equals( re.methodName ) ){
                  ...
                  for( int p = 0; p < numParams; p++ ){
                     try{
                        if( re.methodParameters[p] == null )
                           ...
                           methodParameters[p] = parseParameter( re.methodParameters[p], parameterTypes[p] );
                     }catch( NullPointerException npe ){
                        // debug("Got npe: " + npe);
                        nextMethod = true;
                        break;
                  ...
                  try{
                     ...
                     if( e instanceof InvocationTargetException || e instanceof IllegalAccessException || e instanceof NullPointerException )
                        throw e;
            ...
            debug( "--------------------------------------------" );
            if( !matched ){
               String err = "Method [" + mTargetClass.getCanonicalName() + "." + re.methodName + "()] could not be found matching given parameters";
               if( mTarget == null )
                  err = "Static " + err;
   ...
   /*
    * result handler child class may override this method to have their own result handling
    */
   protected void handleResult( RowEntry re, Object actual, Class<?> returnType ) throws Exception {
   ...
   /*
    * child class can put their code here to log or print debug messages
    */
   ...
   /*
    * child class can put their code here to do cleanup and special exception handling
    */
